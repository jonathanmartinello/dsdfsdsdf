...
// if ($gebruiker['rank'] < 5 && $_GET['shopitem'] == 'attacks')  exit(header('LOCATION: ./index.php?page=market'));

if ($gebruiker['pagina'] == 'attack' && $_COOKIE['market_battle_used'] == '1') exit(header("LOCATION: ./index.php?page=attack/wild/wild-attack"));
...
if($gebruiker['admin'] < 1){
	//echo '<div class="blue">Market em manutenção!</div>';
	//break;
}
...
echo addNPCBox(5, $txt['title_npc'], $txt['desc_npc']);
?>
<div class="orientation-bar" id="itens" style="margin-bottom: -1px">
	<a href="./index.php?page=market&shopitem=balls" data-orientation="balls" class="noanimate"><button type="button"><?=$txt['button_pokeball'];?></button></a>
	<a href="./index.php?page=market&shopitem=items" data-orientation="items" class="noanimate"><button type="button"><?=$txt['button_key_items'];?></button></a>
	<a href="./index.php?page=market&shopitem=specialitems" data-orientation="specialitems" class="noanimate"><button type="button"><?=$txt['button_specialitems'];?></button></a>
	<a href="./index.php?page=market&shopitem=potions" data-orientation="potions" class="noanimate"><button type="button"><?=$txt['button_potions'];?></button></a>
	<a href="./index.php?page=market&shopitem=stones" data-orientation="stones" class="noanimate"><button type="button"><?=$txt['button_stone'];?></button></a>
	<a href="./index.php?page=market&shopitem=pokemon" data-orientation="pokemon" class="noanimate"><button type="button"><?=$txt['button_pkm'];?></button></a>
	<a href="./index.php?page=market&shopitem=attacks" data-orientation="attacks" class="noanimate"><button type="button"><?=$txt['button_tm'];?></button></a>
	<a href="./index.php?page=market&shopitem=others" data-orientation="attacks" class="noanimate"><button type="button"><?=$txt['button_others'];?></button></a>
</div>
<?php
#Pagina's opbouwen
switch($_GET['shopitem']) {
	...
	case "balls":
		...
		if (isset($_POST['balls'])) {
				...
				if ($aantal > 0) {
					...
					if (!is_int($aantal))	$niksingevoerd = true;
						...
						/*} else if (!ctype_digit($aantal)) {
							...
							$rekening_gold -= $goldd;
							$ruimteover -= $aantal;
							$totalitems .= "".$aantal."x ". $itemgegevens['naam'] . ",";
			...
			#silver opslaan
			RemoveMoney($_SESSION['acc_id'], $_SESSION['id'], $totalesilver, "Silver", $_GET['page'], "Comprou ".$totalitems."");
			RemoveMoney($_SESSION['acc_id'], $_SESSION['id'], $totalegold, "Gold", $_GET['page'], "Comprou ".$totalitems."");
	...
	case "items":	#Als er op items geklikt word. Het volgende laten zien 
		...
		if (isset($_POST['items'])) {
			...
			if (empty($_POST['productnaam']))	$niksingevoerd = true;
				...
				$welingevoerd = false;
				echo '<div class="red">'.$txt['have_bought'].'</div>';
			} else {	#Alles is goed
				$welingevoerd = true;
				...
				if ($type[1] == "box") { 
					if ($itemgegevens['naam'] == $bag_allowed) {
						...
						$welingevoerd = false;
						echo '<div class="red">'.sprintf($txt['other_bag'], $bag_allowed).'</div>';
					}
				}
				...
				if (!$itemboxvol) {	#Als itembox niet vol is
					#Opslaan
					RemoveMoney($_SESSION['acc_id'], $_SESSION['id'], $itemgegevens['silver'], "Silver", $_GET['page'], "Comprou ".$itemgegevens['naam']." 1x.");
					RemoveMoney($_SESSION['acc_id'], $_SESSION['id'], $itemgegevens['gold'], "Gold", $_GET['page'], "Comprou ".$itemgegevens['naam']." 1x.");
...
<?php
		break;
	case "others":
		$sql = "SELECT `id`,`naam`,`silver`,`gold`,`omschrijving_" . $_COOKIE['pa_language'] . "` FROM `markt` WHERE `soort`='others' AND `beschikbaar`='1' ORDER BY silver,gold";
		$result = query_cache($_GET['page'] . '_' . $_GET['shopitem'], $sql, $att_time);
...
?>
		...
		<tfoot><tr><td align="center"><input type="submit" name="others" value="<?=$txt['button_spc_items'];?>" class="button" style="margin: 6px"/></td></tr> </tfoot>
	</table></div>
</form>
<?php
		break;
...
<?php
		if ($aantal_paginas > 1) {
			...
			if ($subpage == 1)	echo '<span class="disabled">&laquo;</span>';
				...
				$back = $subpage-1;
				echo '<a href="index.php?page='.$_GET['page'].'&shopitem='.$_GET['shopitem'].'&subpage='.$back.'">&laquo;</a>';
			}
			for($i=1;$i<=$aantal_paginas;++$i) {
				if (3 >= $i && $subpage == $i)	echo '<span class="current">'.$i.'</span>';
				else if (3 >= $i && $subpage != $i)	echo '<a href="index.php?page='.$_GET['page'].'&shopitem='.$_GET['shopitem'].'&subpage='.$i.'">'.$i.'</a>';
				else if ($aantal_paginas-2 < $i && $subpage == $i)	echo '<span class="current">'.$i.'</span>';
				else if ($aantal_paginas-2 < $i && $subpage != $i)	echo '<a href="index.php?page='.$_GET['page'].'&shopitem='.$_GET['shopitem'].'&subpage='.$i.'">'.$i.'</a>';
				else {
					$max = $subpage + 3;
					$min = $subpage -3;  
					if ($page == $i)	echo '<span class="current">'.$i.'</span>';
					else if ($min < $i && $max > $i)	echo '<a href="index.php?page='.$_GET['page'].'&shopitem='.$_GET['shopitem'].'&subpage='.$i.'">'.$i.'</a>';
					else {
						if ($i < $subpage) {
							if (!$links) {
								echo '...';
			...
			if ($aantal_paginas == $subpage) echo '<span class="disabled">&raquo;</span>';
				...
				$next = $subpage+1;
				echo '<a href="index.php?page='.$_GET['page'].'&shopitem='.$_GET['shopitem'].'&subpage='.$next.'">&raquo;</a>';
			}
			echo '</div></td>';
		}
?>
	   ...
	   // header('location: ./index.php?page=market');
		...
		if (isset($_POST['pokemon'])) {
			...
			if (empty($_POST['productid']))	echo '<div class="red">'.$txt['alert_nothing_selected'].'</div>';
				...
				RemoveMoney($_SESSION['acc_id'], $_SESSION['id'], $itemgegevens['silver'], "Silver", $_GET['page'], "Comprou ".$query['naam']." 1x.");
				RemoveMoney($_SESSION['acc_id'], $_SESSION['id'], $itemgegevens['gold'], "Gold", $_GET['page'], "Comprou ".$query['naam']." 1x.");
				echo '<div class="green">'.$txt['success_market'].' '.$txt['success_bought_pokemon'].'</div>';      
			}
		}
?><br>
<div class="red">Aviso: Em caso de compra e o pokémon não vir de seu gosto, não haverá edit, muito menos devolução de Golds.<br>Os pokémons poderão vir entre <b>ACM A EXP</b>, com os IVs entre 15~31, nao nos responsabilizaremos por sua falta de sorte!</div>
<form method="post">
	<div class="box-content" style="position: relative"><table class="general" width="100%">
		<thead><tr><th><?=$title;?></th></tr></thead>
		<tbody>
...
<?php
		break;
	default:	exit(header("LOCATION: ./index.php?page=market&shopitem=balls"));  break;
}
?>
